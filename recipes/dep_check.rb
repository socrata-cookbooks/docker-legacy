...
when 'mac_os_x'
  ...
  unless vbox_exists || vbox_in_runlist
    alert_on_error DockerLegacyCookbook::Exceptions::MissingDependency, action, <<-MSG
...
when 'debian'
  ...
  unless ::Chef::VersionConstraint.new('>= 3.8').include?(node['kernel']['release'].match(/\d+.\d+/)[0])
    alert_on_error DockerLegacyCookbook::Exceptions::InvalidKernelVersion, action, <<-MSG
...
when 'rhel'
  ...
  unless node['kernel']['machine'] == 'x86_64'
    alert_on_error DockerLegacyCookbook::Exceptions::InvalidArchitecture, action, <<-MSG
...
More Info: http://docs.docker.io/installation/rhel/
  ...
  unless ::Chef::VersionConstraint.new('>= 6.5').include?(node['platform_version'])
    alert_on_error DockerLegacyCookbook::Exceptions::InvalidPlatformVersion, action, <<-MSG
